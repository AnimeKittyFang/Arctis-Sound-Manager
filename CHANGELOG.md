...
- **Clips records where the desktop has no ScreenCast portal.** It built one pipeline whose source was a portal session, so XFCE, MATE, Cinnamon and plain window managers — anything using `xdg-desktop-portal-gtk`, which does not implement that interface — could not record at all. X11 is now captured directly there, with the same encoder, the same frame rate and the same per-channel audio tracks. Nothing to configure: the portal is asked whether it exists rather than inferred from the session type, because X11 under GNOME or KDE has a working one and must keep using it. ([#214](https://github.com/loteran/Arctis-Sound-Manager/issues/214))
- **Bug reports say how Clips can capture this screen** — the desktop, which portal backends are installed, whether ScreenCast answers, and which GStreamer elements and encoders exist. Only when Clips is switched on. There is also `scripts/diagnose-clips-capture.py` for anyone who cannot open the window to produce a report in the first place. ([#215](https://github.com/loteran/Arctis-Sound-Manager/issues/215))

### Changed

- **A headset's control interface is found from what it declares, not from a number written by hand.** SteelSeries' own device specifications address it the way Windows does — by HID usage page — and never carry the interface number Linux needs, so every profile's was transcribed, and several were wrong. The number is now read back from the device's report descriptor. A profile that already names the right interface is untouched, so nothing that works today changes. ([#213](https://github.com/loteran/Arctis-Sound-Manager/issues/213))

### Fixed

- **The Arctis Pro GameDAC has its own profile.** It shared one with the Arctis 7 dongle and inherited all of it: an interface that does not exist on this device, so every command failed twice a second and its status came back empty; a battery request, which a mains-powered DAC has none of; ChatMix on the wrong command; and the wrong sidetone opcode. Read from SteelSeries' specification for this product rather than carried over by name. ([#213](https://github.com/loteran/Arctis-Sound-Manager/issues/213))
- **The ChatMix dial no longer skips whole ranges.** The read loop waited on every interface at once, so the slowest set the pace for all of them: the dial pushes its frames on one while another blocks for a full second on a polled status frame. Reported on a Nova 7 Gen 2, where the wheel jumped from 100 to 53 rather than moving through the values. ([#211](https://github.com/loteran/Arctis-Sound-Manager/issues/211))
...
- **ASM no longer asks whether audio should go through Sonar — it just does.** A dialog used to offer, once per installation, to make a Sonar channel your default output. It was asking permission for the thing ASM exists to do, seconds after launch, before you had seen a single channel; and it read the audio graph only, with no idea whether the headset was switched on, so with a wireless headset off it offered to route audio into a channel pointing at a device that was not there — which would have taken the sound away. The daemon has adopted the default on every connect since 1.2.28 anyway, with `redirect_audio_on_connect` in Settings to turn it off, and unlike the dialog it checks the headset is actually on first.
...
- **Clips were labelled after whatever browser happened to be playing.** A clip taken while Chrome had a tab open came out named "Google Chrome", and the Clips page announced it as what was being recorded — to a user who had picked their game's window in the screen picker. Two faults met there. The list of applications a clip is never about was matched against whole names, so it caught the bare binary name `chrome` and missed the `Google Chrome` that PulseAudio actually reports; the same hole let OBS, Brave and Firefox through under their display names. And the name itself was labelled "Recording:", which it never was — it comes from the audio graph, and what is on screen was chosen in the portal picker, which Wayland never reveals to us. The list now matches on words, so display names are caught without a game called *Observation* being mistaken for OBS, and the label reads "Game:".
- **Everything that followed the system default was filed as game audio.** ASM makes one of its channels the default output when the headset connects, so a browser, a music player, system sounds — anything not routed by name — lands there. That channel was Game, not Media: the constant naming it said "media" and held `Arctis_Game`, because the headset's ChatMix dial reports its own two halves as "media mix" and "chat mix" and its "media" half *is* Game. So a podcast was equalised as game audio and then balanced against Discord by the ChatMix dial. The default is now the Media channel; the dial keeps driving Game, which is what it is for.
- **The microphone EQ could be missing entirely, on an install that never opened the Sonar tab.** `effect_output.sonar-micro-eq` is created by a filter-chain config that nothing guaranteed: the config generator covered Game, Media, Chat and Output and skipped the microphone, no package shipped the file, and its only writer was the Apply button in the mic EQ tab. Meanwhile the daemon makes that node the default input at every startup. So on an install where nobody had pressed Apply, the headset microphone was absent from the selectable inputs, and the mic input-source setting and auto-switch did nothing at all. The daemon now writes a flat config when the file is missing — never over an existing one, which would drop your mic processing.
- **The Output channel's device disagreed with itself across three tabs.** Setting it in the Equalizer left Channels and Settings showing the old device, and the other way round. Two causes met: the Equalizer's picker kept its own preference file that nothing else read, and the two sides did not even spell a device the same way — one saves a `node.nick`, the other keys its list on `node.name`, so a value that did travel could still fail to match. One setting is the source of truth now, both spellings resolve, and a fresh pick is held until it is saved rather than being overwritten by a stale read.
...
- **SteamOS: the Distrobox installer stopped part way through, looking as though it had worked.** `steamos-readonly disable` needs root and was called without it, so it failed, the udev step returned an error and the install stopped — after printing the steps it had already completed. Reported and diagnosed by [@emoeckel](https://github.com/emoeckel). ([#188](https://github.com/loteran/Arctis-Sound-Manager/issues/188))
- **The .deb on the releases page was missing pieces every other package had.** Debian is built two ways — `debian/rules` for the PPA, `debian/build-deb.sh` for the release attachment — and the second had drifted: no `arctis-stream-guard` unit, and a hand-kept list of console scripts holding five of the eight entry points, so `asm-stream-guard` and `asm-clipd` were not in the package at all. A unit file and a keybinding pointed at executables that did not exist. The list is now read from `pyproject.toml`, and a test compares the two build paths.
...
- **Profile fixes now reach you even if you installed ASM months ago.** This one was silently cancelling out a lot of the work in recent releases. At first launch, ASM copied every device profile into your own configuration folder — and that copy takes precedence over the packaged one, forever, because no update ever refreshed it. So every profile fix shipped after your first run was inert for you: new headsets ASM had just learned to drive, corrected status byte offsets, fixed EQ commands, none of it landed. You would update, see the new version number, and keep running the protocol of the release you first installed. If you have been reporting a problem that "still isn't fixed" after several updates, this is very likely why. The daemon now reconciles that folder at every start: a copy identical to the packaged profile is removed, one you modified is moved aside to `devices-disabled/` and named in the log so nothing of yours is destroyed, and a profile for a headset ASM doesn't ship is left strictly alone.
- **Arctis Nova 7 / 7P Gen 2, Nova 5, Nova 3 Wireless, GameBuds: the custom EQ sliders did nothing at all.** Every equaliser frame went out one byte off, so the headset read an opcode of zero and discarded the curve without a word. Confirmed on hardware: a Nova 7P Gen 2 reported its untouched factory preset while a custom curve was set in the app. The readback decoder was misreading its answer by one byte too, and both are now pinned against real replies from the headset.
- **Arctis Nova Pro Wireless and Nova Pro Wired GameDAC: a custom EQ you could see and not hear.** The base station drew your curve on its screen as you moved the sliders — proof the command was arriving — while the sound never changed, because the headset kept applying its Flat preset and nothing ever switched away from it. The curve is now selected once it has been written. If you never touch these sliders, nothing changes: the headset stays neutral behind the Sonar EQ exactly as before.
- **The two equalisers no longer stack.** ASM equalises either in software (Sonar mode) or in the headset (Custom EQ mode), never both — but only the software half of that was enforced. Switching to Sonar left the on-device curve applied underneath it, so neither setting meant what it said. The hardware equaliser now follows the mode you chose.
- **Pop!_OS and Ubuntu 22.04: ASM would not start at all on a headset with a display.** The daemon crashed while setting up the OLED — on a version of the Pillow imaging library older than the one it assumed — and because that happens while the audio pipeline is being built, it took everything down with it: no mixer, no EQ, no device settings, not just no screen. It now works with the Pillow versions those releases ship, and a display failing can no longer stop the rest of ASM from running. (#154)
- **Arctis Nova 3P / 3X Wireless: the Microphone section was missing from the Devices page.** Nothing was wrong with reading the mic state — the headset only announces it when it changes, and if you had not pressed mute since starting ASM there was nothing to show, so the whole section stayed hidden. With the boom mic unplugged, pressing mute reported nothing either, leaving no way at all to make it appear. ASM now asks the headset for the mic state at startup instead of waiting to be told. (#149)

## [1.2.18] - 29 July 2026

### Fixed

- **Custom EQ: your saved curve now reaches the headset at startup.** On the Nova 7 / 7P Gen 2, Nova 5, Nova 3 Wireless and GameBuds, the curve stored in the app was replayed at every start in a format only the Nova Pro Wireless understands, so the headset discarded it without a word. The custom EQ was effectively inert after each restart and only came alive if you happened to move a slider by hand.
- **Arctis Nova 3P / 3X Wireless: the ChatMix dial should now report to the app.** Turning the wheel produced no USB traffic whatsoever — it was acting as a plain local volume knob. On this dongle ChatMix is a mode the computer has to switch on, and ASM never did; it does now. (If your dial still does nothing after this update, please say so in #149 — this one could not be tested on hardware here.)
- **Bluetooth indicator no longer shows a state the headset never sent.** On the Nova 7 Gen 2, Nova 7P Gen 2 and Nova 5, the connection indicator read a byte SteelSeries' own specification marks as unused, so what it displayed was padding dressed up as a link state — on the Nova 5, a headset that has no Bluetooth at all. The row is gone rather than wrong.
- **Arctis Nova 7 WoW Edition (post-update firmware): battery percentage was wildly wrong.** That dongle's PID was filed with the first-generation Nova 7, whose battery is reported on a four-step scale. After its firmware update the headset speaks the Gen 2 protocol and reports a real 0-100 %, so a battery at 76 % came out as 1900 %. It is now read with the right family.

### Added

- **`asm-cli tools read-hardware-eq` — read the equaliser curve back out of the headset.** For the Nova 7 / 7P Gen 2. When the custom EQ sliders seem to do nothing (#146), this tells apart the three cases that look identical from the outside: the curve arrived and is stored, the curve arrived and the headset ignored it, or nothing reached the headset at all. It also prints the raw reply, so a decoding that turns out to be off can be corrected from a real report rather than a guess.
...
- **An update now actually takes effect, instead of waiting for your next reboot.** A package manager replaces files on disk; it does not touch what is already running. ASM went on executing the version it had loaded at startup — so someone who updated *because* of a fix still saw the bug, and reported it against a version number they were not running. The background services are now restarted at the end of the update, on Arch, Fedora and Debian alike. The window, which your desktop starts rather than the system, notices on its own and offers a restart: **"Version X is installed — restart ASM to run it"**. Nothing of the sort appears for anyone running from a source checkout.
- **The in-app update no longer insists on an AUR helper** on Arch, CachyOS and derivatives. It proposed `paru -S` whatever the origin of the package; installed from a repository, a plain `pacman -Syu` does the job and your system's own updater picks it up unprompted. ASM now asks pacman which of the two situations it is in.

### Added

- **ASM can be installed from a signed binary repository on Arch, CachyOS, EndeavourOS and other Arch derivatives** — no AUR helper, no compilation, and updates arriving with the rest of the system. Packages and database are signed with the project's key, and the repository is rebuilt on every release.
  ...
  The installer (`install.sh`) now prefers it on Arch and falls back to the AUR. Existing AUR installs are unaffected and keep updating as before.
- **ASM can appear in graphical software centres** — Discover, GNOME Software and other AppStream front-ends. The file describing the application named no package, which is what those tools match on, so the entry existed and connected to nothing: a search returned an empty list even where the package was perfectly installable. The packaging now ships that link, on all three distributions. (Arch users also need `packagekit`, and `packagekit-qt6` on KDE — neither is installed by default, and the README says so.)
...
- **Your headset's own equaliser is now used, on nine models.** Until now the custom EQ was applied by the computer, and only ever reached the hardware on the Nova Pro Wireless and Wired — on every other model it did nothing at all. The curve is now set inside the headset on the Nova 7 Gen 1 and Gen 2, Nova 7P, Nova 5, Nova 4 and Arctis GameBuds as well. That means it follows the headset to a console, a phone or another computer, costs no CPU, and stays active even when ASM isn't running. Each family speaks a different dialect for this — different opcodes, different frame layouts, some needing a pause between writes — all of which is now handled per model. **Not yet verified on hardware for the newly added families: if your equaliser sounds wrong rather than absent, please say so in [#146](https://github.com/loteran/Arctis-Sound-Manager/issues/146).**
- **The Nova Pro Omni and Nova Pro Wireless expose their Bluetooth preferences**: whether Bluetooth comes up with the base station, and what an incoming call does to game audio (nothing, lower by 12 dB, or mute). Both were previously forced to a fixed value at every start, silently undoing whatever you had set from the DAC itself or from SteelSeries GG.
- **The Nova Pro Omni's microphone noise reduction** — done inside the base station, rather than by a plugin on your computer.
- **ASM now asks the headset what it already has** instead of assuming, on the Nova 7 Gen 2 family. A headset configured elsewhere — in GG on a dual-boot machine, or from its own controls — used to be reset to this app's defaults the first time ASM ran. Its own values are now adopted for anything you have never set here; anything you have set stays yours.

### Fixed

- **The Noise Cancelling section no longer appears on headsets that don't have it.** It sat there with controls that could not do anything, which reads as a broken app rather than an absent feature. (#146, reported by @camperotactico)
- **The mixer's help tooltip is readable again.** It rendered as a plain black rectangle on dark themes: the app styled every other widget but never tooltips, so their text came out dark on dark. Fixes every tooltip in the app. (#146)
- **Outputs no longer go missing from the device lists.** Two optional PipeWire properties were required to list a sink, and one declaring neither was dropped without a word — which is how ordinary desktop speakers ended up "not in the list". (#146, and the same cause as the Bluetooth speakers of #134)
- **The battery reading no longer implies precision the hardware doesn't have** on the Nova Pro Wireless. It reports nine levels, not a percentage, so its readout showed values like 62 % for something measured in steps of about twelve points. It now snaps to round numbers. Headsets that do measure to the point are untouched.
- **The Nova 5's sidetone slider had a dead zone**: its top third sent levels the headset has no meaning for.
...
- **The Arctis Nova Pro Omni's controls finally work.** Its profile was a Nova Pro Wireless copy, and the DAC quietly refused almost everything ASM sent it: commands went out with the wrong HID report id, so the base station dropped them without a word — which is why sidetone, the volume limiter and the DAC screen looked dead however you set them. Huge thanks to **@joorklee**, who reverse-engineered the real protocol on hardware (a Windows capture of SteelSeries GG driving the DAC, GG's own device specification, and Linux writes confirmed by reading the values back off the base station) and shared everything in #147. Sidetone, the volume limiter, ANC and its level, transparency, mic volume and LED, the OLED screen and the auto-off timer now speak the language this DAC actually understands, and the headset's battery, charging state and Bluetooth status are reported as the device really sends them instead of being guessed from a sibling model's layout. The base station also pushes changes as they happen, so what you do on the DAC shows up in ASM without waiting for the next poll. (#70, #147, protocol by @joorklee)
- **Arctis Nova 7P Gen 2 (`1038:2298`) is now recognised out of the box.** (#146, reported by @camperotactico)
- **New Sonar presets show up the day they are published.** The check ran at most once every 24 hours, so a preset pack released shortly after your last check stayed invisible for a full day and reopening the app changed nothing. ASM now looks for new presets on every launch.

### Fixed

- **A device you added yourself was ignored when writing the udev rules.** The documented way to support a headset ASM doesn't know yet is to drop a copy of its configuration file into your own configuration folder and add the product id there. That copy drove device detection, but the bundled file silently won when the rules were generated — so the new product id never reached the rules file, with no error to explain why nothing happened. Your copy now wins everywhere. (#146)
- **`sudo asm-cli udev write-rules` looked in root's home instead of yours**, which put your own device files out of reach entirely — the same symptom, from the other end. The command reads the invoking user's folder when elevated (and still never writes anything into your home as root). It doesn't need `sudo` in the first place: it asks for the password itself when it needs one. (#146)
- **`asm-cli diagnose` now warns when it is run as root.** A dump taken with `sudo` describes root's session — no audio server, no settings, no device files — which looks exactly like a badly broken installation and sends troubleshooting down the wrong path. (#146)
- **An unreadable device folder no longer aborts the whole udev rules generation.**
...
- **Apps routed to the "Output" channel could play into silence.** The last hop of that channel — its equaliser output into the external sink (HDMI, TV, speakers) — was the only one in ASM that belonged to nobody: it was not covered by the link ownership applied to Game and Media, nor by the watchdog that re-establishes the Chat and surround outputs. Every filter-chain restart tore that link down and nothing ever put it back, so an application pinned to the Output channel kept playing into a dead end, with no sound and no error message. The watchdog now enforces this hop too, following the external output you selected.
- **Releasing the headset no longer leaves USB interfaces in limbo.** ASM claims every USB interface it uses when it takes the device over, but on teardown it released only one of them before handing the device back to the kernel driver — which cannot take back an interface still held by another process. Should that happen while switching between two different headset models, the affected interfaces ended up owned by neither ASM nor the kernel until the device was replugged. Claim and release now cover the same set.
- **A channel could stay silent for minutes after the headset was plugged back in.** The watchdog's anti-flapping protection — which deliberately backs off from a channel that keeps failing — tracked channels by name, and that name outlives the process behind it. A channel that had been struggling shortly before you switched the headset off was therefore still being skipped when it came back, for up to five minutes, even though its process was brand new. The protection now resets whenever the headset session is rebuilt, while continuing to back off exactly as before within a session.
- **Apps pinned to a channel stopped following the headset after a replug.** Reconnecting the headset rebuilds all three virtual channels, which drops every application you had pinned to them back onto the system default output. Only the Chat channel was restoring those pins, so pins on Game and Media were silently lost — after a replug or a channel restart, browsers, games and video players simply stopped following the headset. Pins are now replayed on every path that rebuilds a channel.
- **Brief audio dropouts when the audio graph changed underneath ASM.** Each watchdog pass resolved its PipeWire connections from a single snapshot taken at the start of the cycle; when ports moved in between — a WirePlumber restart, an audio reconfiguration — the links it tried to make pointed at things that no longer existed, and part or all of a channel went unlinked until the next cycle five seconds later. A failed pass now retries immediately against a fresh snapshot, and the three connections that previously had no recovery path at all (surround, physical output, microphone) escalate to a filter-chain repair if they stay broken.
- **The "Output" channel disappeared from application output pickers once it had an active equaliser.** Its sink was declared as an internal node in the stereo case, unlike every other code path that builds it, so applications could no longer see it and a saved routing pin to that channel could not be restored. It is now a regular output everywhere, whatever its channel count.
...
- **Changing the EQ or toggling Spatial Audio no longer restarts the audio engine.** Adjusting EQ gains, the macro sliders or the boost — and switching Spatial Audio on or off — is now applied live to the running filter-chain instead of restarting it. On PipeWire 1.6.7 that restart could crash the filter-chain and briefly cut Game/Media audio or leave Spatial Audio silent; these changes now take effect instantly with no dropout. Changes that genuinely alter the graph (adding/removing an EQ band, switching preset, changing the HeSuVi immersion/distance) still restart as before. (#100, #88)
- **Fixed a rare daemon crash-loop that produced random `SIGSEGV`s.** The daemon spawned its helper processes (`pw-dump`, `pw-loopback`, `pw-metadata`, `systemctl`…) with a `fork()` that, while the daemon was talking to the headset over USB in another thread, could corrupt memory and crash it with nonsensical errors (seen as a 10-crash restart loop). Every helper spawn now uses a fork-free `posix_spawn` path. (#123)
- **The tray battery percentage now disappears when the headset is powered off.** The wireless adapter keeps reporting a battery level even when the headset is off, so the tray number used to stay on screen; it now hides once the headset powers down. (#124)

### Internal

- Added a Distrobox troubleshooting section to the README for silent Spatial Audio / audio cut-outs inside containers (host-vs-container PipeWire version skew, and the filter-chain 1.6.7 crash on restart). (#100)
...
- **pacman mirror 404 auto-retry in deps dialog** — when installing a missing dependency via the system deps dialog on Arch/CachyOS, a desynchronized mirror (HTTP 404 on the package file) caused the install to fail silently. The dialog now captures `pacman` output, detects the mirror-sync error pattern, and automatically retries with `pacman -Syy` to force a database refresh before running the original install command. A guard flag prevents infinite retry loops.
